...
import java.util.*;                                      // Import utilities for List, Set, Arrays, etc.
import java.util.stream.*;                               // Import Stream and IntStream APIs for Java 8 usage;
...
public class AllPairsSumX {                              // Main public class to hold solutions and test harness
    ...
    static final class Pair {                            // Define a small helper class representing a pair (a, b)
        ...
        Pair(int a, int b) { this.a = a; this.b = b; }   // Constructor to set both fields
        @Override public String toString() {             // Override toString for readable output
            return "(" + a + "," + b + ")";              // Format pair as (a,b)
        }
        @Override public boolean equals(Object o) {      // Override equals for use in collections if needed
        ...
        @Override public int hashCode() {                // Override hashCode consistent with equals
    ...
    private static void shuffleArray(int[] a, long seed) {                          // Shuffle helper to avoid ordered bias
        ...
        for (int i = a.length - 1; i > 0; i--) {                                    // Walk backward through the array
            ...
            int tmp = a[i]; a[i] = a[j]; a[j] = tmp;                                // Swap a[i] and a[j]
        }
    }
}