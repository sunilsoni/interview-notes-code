...
class StandardHotel implements Hotel { // Class implementing the required Hotel interface
    private final Map<Integer, Room> directory = new HashMap<>(); // UPDATED: Map now uses Integer keys for O(1) lookups
    ...
    @Override // Override notation for interface implementation
    public boolean bookRoom(int roomId, LocalDate checkIn, LocalDate checkOut) { // UPDATED: Takes integer roomId
        ...
        targetRoom.addBooking(checkIn, checkOut); // Delegate the booking action directly to the target Room object
