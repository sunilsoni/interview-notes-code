package com.interview.notes.code.year.y2026.april.Wallmart.test3;
...
public class ActiveSessionTracker { // Defines the main class for the solution
        ...
        List<TestCase> tests = List.of( // Java 9+ List.of creates an immutable list of our test scenarios
            new TestCase( // Initializes the primary test case handling Raja's failure
                List.of( // Defines the ordered list of application logs
                    ...
                    "10:20 Raja ERROR FAIL", // Raja logout fails -> Ignored, State remains Active
                    "10:25 UserD LOGIN SUCCESS" // UserD logs in -> State: Active
                ), // Closes log list
                Set.of("Raja", "UserC", "UserD") // Expected output: Raja must be present due to failed logout
            ) // Closes test case
    ...
    public static Set<String> getActiveUsers(List<String> logs) { // Core business logic method to process sessions
        return logs.stream() // Initiates a sequential Java 8 Stream over the log entries
            ...
            .filter(arr -> arr.length >= 4) // Filters out any malformed data to prevent IndexOutOfBounds exceptions on large datasets
            .collect( // Collects the stream output into our final data structure
                ...
                (activeSet, arr) -> { // BiConsumer defines exactly how to process each log array into the Set
                    ...
                    if ("SUCCESS".equals(status)) { // Explicitly checks if the operation was successful before altering state
                        switch (action) { // Java 21 switch expression cleanly routes the action
                            case "LOGIN" -> activeSet.add(user); // If successful login, add user to active set
                            case "LOGOUT" -> activeSet.remove(user); // If successful logout, remove user from active set
                        } // Closes switch
                    } // Closes if statement; failed operations natively do nothing, leaving state intact
