package com.interview.notes.code.year.y2024.nov24.amazon.test3;
...
public class TemperatureRegulator {
    ...
    public static long regulateTemperatures(List<Integer> temperature) {
        ...
        for (int i = 1; i < adjustedTemps.size(); i++) {
            if (adjustedTemps.get(i) > adjustedTemps.get(i - 1)) {
                operations_via1 += (adjustedTemps.get(i) - adjustedTemps.get(i - 1));
            }
        }
        ...
        for (int i = adjustedTemps.size() - 2; i >= 0; i--) {
            if (adjustedTemps.get(i) > adjustedTemps.get(i + 1)) {
                operations_via2 += (adjustedTemps.get(i) - adjustedTemps.get(i + 1));
            }
        }
    ...
    public static void main(String[] args) {
        ...
        testCases.add(new TestCase(Arrays.asList(-1, -2, -3, -4, -5), 5L + 5L)); // z=5, operations_via1_and_2=5
        ...
        testCases.add(new TestCase(Arrays.asList(3, 1, 4, 1, 5, 9, 2, 6, 5), 15L));
