...
import java.util.ArrayList; // Imports ArrayList to build our dynamic list of merged results
...
public class IntervalMerger { // Defines the main class that encapsulates the interval logic and test runner
    ...
    public static List<Interval> merge(List<Interval> intervals) { // Defines the static method that takes an unmerged list and returns a cleanly merged list
        if (intervals == null || intervals.isEmpty()) { // EDGE CASE 1: Checks if the input is missing or empty to prevent NullPointerExceptions
            return List.of(); // Returns an immutable empty list immediately since there is nothing to merge
        ...
        var sorted = intervals.stream() // Converts the input list into a Stream API pipeline to apply functional sorting logic easily
            .sorted(Comparator.comparingInt(Interval::start)) // EDGE CASE 4: Sorts all intervals in ascending order based purely on their 'start' property so we can safely sweep them
        ...
        for (var next : sorted) { // Begins an enhanced for-loop to iterate through every single interval in our newly sorted list
            if (next.start() <= current.end()) { // EDGE CASE 3: Checks for overlap or exactly adjoining edges (<=) by seeing if the new start touches or precedes the tracker's end
                current = new Interval(current.start(), Math.max(current.end(), next.end())); // EDGE CASE 2: Creates a new merged interval maintaining the start, but expanding the end to the maximum reach of both (handles swallowed intervals)
    ...
    public static void main(String[] args) { // Defines the standard application entry point used here to execute our custom testing framework
        runTest("Empty/Null Check", List.of(), List.of()); // Tests the edge case where absolutely no intervals are provided
        ...
        runTest("Fully Contained (Swallowed)", // Initiates a test where one interval is completely swallowed by another
        ...
        runLargeDataTest(); // EDGE CASE 5: Triggers a separate custom method strictly designed to test performance and memory on massive datasets
    ...
    public record Interval(int start, int end) {} // Uses Java 16+ Record to create an immutable data class, removing boilerplate getters completely
