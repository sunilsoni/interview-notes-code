...
public class Solution {
    ...
    /**
     * maximizeGroups:
     * Given a list of product counts (each count indicates how many items are available
     * of that product type), we want to form batches such that the 1st batch uses 1 item,
     * the 2nd uses 2, the 3rd uses 3, etc. In each batch, the items must come from distinct
     * product types (so one product can contribute at most one item per batch).
     *
     * A candidate m (number of batches) is feasible if we can assign items from the products
     * to cover batches 1,2,…,m. Because a product type can only supply one item per batch,
     * its total contribution is capped at m. Thus a necessary and sufficient condition is:
     *
     *    sum( min(products[i], m) for all i ) >= m*(m+1)/2.
     *
     * In this version we sort the list and build a prefix sum so that for any candidate m we can
     * quickly compute the left‐side.
     *
     * Time Complexity: O(n log n) for sorting + O(log(max_m)*log n) for binary search.
     */
    public static int maximizeGroups(List<Integer> products) {
        int n = products.size();
        // Sort the product counts in non–decreasing order.
        Collections.sort(products);
        
        // Build prefix sum array (prefix[0] = 0, prefix[i] = sum of products[0..i-1])
        long[] prefix = new long[n + 1];
        for (int i = 0; i < n; i++) {
            prefix[i + 1] = prefix[i] + products.get(i);
        }
        
        // We know that even if all products are huge, each product can contribute at most m,
        // so total contribution is at most n*m. To cover m batches we need m*(m+1)/2.
        // Hence we must have: n*m >= m*(m+1)/2, which implies m <= 2n.
        ...
        while (left <= right) {
            ...
            if (totalContribution >= required) {
                answer = mid;       // candidate m works, try for a larger m
                left = mid + 1;
            } else {
                right = mid - 1;
            }
    ...
    /**
     * computeContribution:
     * Given the sorted products list and its prefix sum, this method returns
     * the sum over i of min(products[i], m).
     *
     * To do so, we find the first index 'idx' at which products[idx] > m.
     * For all indices i < idx, products[i] <= m so min(products[i], m) = products[i].
     * For indices i >= idx, min(products[i], m) = m.
     */
    private static long computeContribution(List<Integer> products, long[] prefix, int m) {
        int n = products.size();
        // Find the first index where product > m.
        int idx = upperBound(products, m);
        // Sum of contributions for indices [0, idx-1]: exactly prefix[idx]
        long sum = prefix[idx];
        // For indices idx ... n-1, each contributes exactly m.
        sum += (long) (n - idx) * m;
        return sum;
    }
    ...
    /**
     * upperBound:
     * Given a sorted list and a value m, returns the first index at which
     * the element is greater than m.
     */
    ...
    /**
     * runTest:
     * A simple helper that runs one test case and prints PASS/FAIL.
     */
    ...
    /**
     * main:
     * Runs several test cases including the provided sample cases and some long–list tests.
     * (For very long lists we only print the computed result so you can compare with your expected answer.)
     */
    public static void main(String[] args) {
        // Sample Case 0:
        List<Integer> test0 = Arrays.asList(1, 2, 7);
        // Expected answer: 3 (as explained in the problem statement)
        runTest(0, test0, 3);
        ...
        // As computed by the condition, the maximum m is 9.
        runTest(2, Arrays.asList(1, 1, 8, 11, 11, 12, 12), 9);
        
        // Provided Test Case B (long list – only a prefix is shown here).
        ...
        // For this long test case, we simply print the computed result.
        System.out.println("Test 3 (long list B) result: " + maximizeGroups(testB));
        ...
        System.out.println("Test 4 (long list C) result: " + maximizeGroups(testC));
    }
}