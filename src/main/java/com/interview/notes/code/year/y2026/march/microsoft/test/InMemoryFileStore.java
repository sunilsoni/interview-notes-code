package com.interview.notes.code.year.y2026.march.microsoft.test;

import java.util.Arrays;
import java.util.HashMap;
import java.util.Map;

// Defines the downstream memory service interface as provided in the problem
interface IMemoryService {
    int allocate(int length); // Reserves a contiguous block; returns 1-based start index or 0 on failure
    byte[] read(int startIndex, int length); // Reads contiguous bytes from memory
...
public class InMemoryFileStore {
    ...
    private int nextId = 1; // Counter to generate unique, strictly positive file IDs

    public InMemoryFileStore(IMemoryService mem) {
        this._mem = mem; // Injects the memory service dependency into our class instance
    }

    // Main method to run all tests
    public static void main(String[] args) {
        System.out.println("Starting Tests..."); // Prints a starting banner
        ...
        // Test 3: Large data inputs (Stress test)
        int id5 = store.Create(1000000); // Allocates a very large file (1 million bytes)
        byte[] largeData = new byte[1000000]; // Creates a large byte array
        Arrays.fill(largeData, (byte)42); // Fills the array with a dummy value
    ...
    public int Save(int fileId, byte[] data) {
        ...
        if (data.length == info.length()) { // Checks if the new data perfectly fits the currently allocated block
            ...
            fileMap.put(fileId, new FileInfo(newStart, data.length)); // Updates our internal directory with the new memory location
    ...
    // Uses Java 14+ record feature to create an immutable data carrier for memory metadata without boilerplate
    private record FileInfo(int startIndex, int length) {} // Holds the starting index and size of an allocated block
