...
public class Main {
    ...
    public static String countDuplicates(String str) {
        ...
        for (char c : str.toCharArray()) { // Converts string to a char array and loops through each character
            ...
            int count; // Declares a variable to hold the calculated frequency for the current character
            if (counts.containsKey(c)) { // Checks if the character already exists in our tracking map
                count = counts.get(c) + 1; // If it exists, retrieves the previous count and increments it by 1
            } else { // Handles the scenario where the character is being seen for the first time
                count = 1; // Initializes the character's count to 1
            } // Ends the if-else frequency calculation logic
            
            counts.put(c, count); // Stores the updated or new frequency back into the map for this specific character
