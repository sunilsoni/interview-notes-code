...
import java.util.PriorityQueue; // We need this to automatically sort and fetch the smallest nodes
...
public class MergeKSortedLists { // Main class containing our logic and testing framework

    public static ListNode mergeKLists(ListNode[] lists) { // Method taking an array of lists and returning one merged list
        if (lists == null || lists.length == 0) return null; // Immediate safety check: if there are no lists, return nothing
        
        // Create a queue that automatically keeps the node with the smallest 'val' at the very front
        PriorityQueue<ListNode> queue = new PriorityQueue<>(Comparator.comparingInt(node -> node.val)); // Java 8 feature: compact way to compare node values
        ...
        Arrays.stream(lists) // Convert our standard array of lists into a Java Stream for processing
              ...
              .forEach(queue::add); // Add the head node of every valid list into our Priority Queue
        ...
        while (!queue.isEmpty()) { // Keep looping as long as we have nodes waiting in our queue
            tail.next = queue.poll(); // Pluck the absolute smallest node out of the queue and attach it to our merged list
    ...
    public static void main(String[] args) { // Standard entry point to run our tests
        ...
        runTest("Test 1: Standard Case", new int[][]{{1, 4, 5}, {1, 3, 4}, {2, 6}}, "1->1->2->3->4->4->5->6"); // Run basic test provided in prompt
        runTest("Test 2: Empty Lists", new int[][]{}, ""); // Run edge case: completely empty input
        runTest("Test 3: Lists with nulls", new int[][]{null, {1, 2}, null, {3, 4}}, "1->2->3->4"); // Run edge case: array contains null elements
    ...
    private static void runTest(String testName, int[][] inputArrays, String expectedOutput) { // Accepts test name, raw data, and expected string
        ...
        var resultNode = mergeKLists(lists); // Execute our core merging algorithm on the generated lists
    ...
    // Helper method to test huge volumes of data (Performance test)
    private static void runLargeDataTest() { // Method specifically for testing high volume inputs
        ...
        for (int i = 0; i < k; i++) { // Loop 1,000 times to create the lists
            ...
            for (int j = 0; j < nodesPerList; j++) { // Loop 50 times to add nodes to this list
                current.next = new ListNode(i + j); // Create nodes with slightly increasing values so they remain sorted
        ...
        var result = mergeKLists(lists); // Execute the merge algorithm on all 50,000 nodes
        ...
        if (count == (k * nodesPerList)) { // Verify the total node count equals 50,000
            System.out.println("Test 4: Large Data (" + count + " nodes): PASS in " + (endTime - startTime) + "ms"); // Print PASS and show execution speed
        } else { // If the count is wrong, data was lost or duplicated
            System.out.println("Test 4: Large Data: FAIL (Count mismatch)"); // Print FAIL
    ...
    // Utility method to convert a Linked List into a formatted string (e.g., "1->2->3")
...
} // Ends the MergeKSortedLists class